...
- **The Town-2 gate is the mountain, with two crossings that open at the Act 2 midpoint** (rev. 2026-07-29). The mountain (the tall terrain **labelled "rural" in the model** — `Rural1`) is the physical barrier between Town 1 and Town 2. Two ways through, both sealed in Act 1:
  - **A bridge**, visibly **under construction in Act 1**, opening in Act 2. The construction is the concrete countdown — you watch it get built.
  - **A road tunnel driven underneath the mountain** (with a metro station at its mouth), **blocked until Act 2**.
  - Whichever opens first *is* the midpoint reward; the player stares at both for an act. Same trick as the closed city metro, and more concrete than an abstract ridge.
- **The metro is ONE line, extended in two stages** (decided 2026-07-29 — option b): the under-mountain line reaches **Town 2 at the Act 2 midpoint**, then extends to **the City in Act 3**. The same line the player first rides out of Town 1 at the midpoint later carries them to the fixer stage — one structure, two reveals, and "knowledge = geography" rides the rails. (Supersedes the earlier "separate city metro, Act 3 only".) In Act 1 the station is sealed; that is the only state the greybox needs.
...
- **Transport spine:** one bus line linking zones; taxis downtown & at gas stops; the **Town-2 crossings** (bridge under Act-1 construction + under-mountain road tunnel/metro, both opening Act 2) and a **metro to the city** (Act 3) — see §2.1 for the open reconciliation of whether these are one line or two. All expansion routes are foreshadowed in concrete: sealed structures you pass for acts before they open.
