...
**Gate 1 passes iff (point-estimate criteria):** blind ROI **point estimate** in band **AND** informed ROI **point estimate** ≥ +0.05. The even-money win-rate 55–60% condition (on the point estimate) is applied **only when** the qualifying subset has ≥ `min_even_money_bets` bets; below that threshold it is reported but NOT required, and Gate 1 rests on the two ROI criteria alone. The fixture-clustered bootstrap **95% CIs are computed and reported for transparency, but are NOT required to sit inside the bands** — this is standard calibration practice: you calibrate to the point estimate, and shrinking a bootstrap CI to fit a 2-point band needs ~100× more samples without changing the (already-correct) calibration. If a required point estimate is missed, tune `constants.json`; do not proceed to Phase 2. (`edge_threshold`, fixture count, bands, `min_even_money_bets`, and the derived `blind_roi` all live in `constants.json`.)

_Note: an earlier draft required the whole CI inside each band; that criterion was demonstrably too strict (infeasible sample sizes) and was relaxed to point-estimate calibration once the engine's point estimates all landed on target (blind ≈ −8.3%, informed ROI positive, even-money win rate 55–60%)._
